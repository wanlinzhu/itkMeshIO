    ...
    if ( m_GiftiImage->darray[ii]->intent == NIFTI_INTENT_POINTSET )
      ...
      switch ( m_GiftiImage->darray[ii]->datatype )
        ...
        default:
	      ...
	      //display giiCoordSystem
	      /*
	      for(unsigned int kk = 0; kk < m_GiftiImage->darray[ii]->numCS; ++kk)
	      {
		      gifti_disp_CoordSystem("mat = ", m_GiftiImage->darray[ii]->coordsys[kk]);
	      }
	      */
      }
      ...
      if ( m_GiftiImage->darray[ii]->num_dim > 0 )
        ...
        /*
        if(this->m_NumberOfPoints == 0 && this->m_NumberOfCells != static_cast<unsigned long>(m_GiftiImage->darray[ii]->dims[0]))
        {
          this->m_NumberOfPoints = m_GiftiImage->darray[ii]->dims[0];
        }
        */
...
void GiftiMeshIO::ReadPoints(void *buffer)
{
  ...
  for ( int ii = 0; ii < m_GiftiImage->numDA; ++ii )
    ...
    if ( m_GiftiImage->darray[ii]->intent == NIFTI_INTENT_POINTSET )
      ...
      //	std::copy(static_cast<char *>(m_GiftiImage->darray[ii]->data),
      // static_cast<char *>(m_GiftiImage->darray[ii]->data) + pointsBufferSize
      // * m_GiftiImage->darray[ii]->nbyper, buffer);
      }
    }
}
